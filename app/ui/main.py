...
class UI:
    ...
    def _send_feedback(self, user_input, api_output, feedback):
        ...
        Args:
            user_input: The user's input.
            api_output: The output generated by the API.
    ...
    def show_main_page(self):
        ...
        with st.form(key="formalizer_form"):
            ...
            with col2:
                clear = st.form_submit_button("Wyczyść")
        ...
        if st.session_state.get("formalized_text"):
            ...
            col1, col2 = st.columns(2)
            with col1:
                if st.button("👍 Podoba mi się", key="like"):
                    self._send_feedback(
                        st.session_state.input_text,
                        st.session_state.formalized_text,
                        "positive",
                    ...
                    st.rerun()
            with col2:
                if st.button("👎 Nie podoba mi się", key="dislike"):
                    self._send_feedback(
                        st.session_state.input_text,
                        st.session_state.formalized_text,
                        "negative",
            ...
            if st.session_state.get("feedback_message") == "positive":
                ...
                st.error(
                    "Dziękujemy za opinię! Twoja opinia pomoże nam poprawić jakość usługi."
                )
